        ...
        groups: [
            {
                ...
                components: [
                    ...
                    snk('xlsx', 'Excel — multi-tab (XLSX)', 'available', "Write rows into one named sheet of an .xlsx workbook via the pure-Rust umya-spreadsheet crate, leaving the other tabs intact. Unlike the DuckDB excel sink - whose COPY recreates the whole file on every write - a second run against the same file keeps existing sheets. Pick a Sheet name and a write mode (Replace the tab, or Append rows beneath it)."),
            ...
            },
            {
                ...
                components: [
                    ...
                    snk('infor', 'Infor', 'available', 'Bulk-upload rows to an Infor FSM/Landmark business-class action (Create / Update / ...) via the batch service. Connect a dataset, pick the action, and map columns to the action fields in the rail; per-record results (status + message) flow out the output for logging or retry.'),
