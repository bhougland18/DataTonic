...
fn allowed(component: &str, key: &str) -> Option<&'static str> {
    match (component, key) {
        ...
        // qa.regex.studio routes through build_quality, which reads columns
        // and tieBreak ONLY in its `qa.unique` branch. The regex studio node
        // takes the predicate path (column/pattern), so it never reads these.
        ("qa.regex.studio", "columns") => Some("read only in build_quality qa.unique branch; qa.regex.studio uses column/pattern"),
        ("qa.regex.studio", "tieBreak") => Some("read only in build_quality qa.unique branch; qa.regex.studio uses column/pattern"),
        _ => None,
    }
}
...
mod plan_arms {
    ...
    fn arm_allowed(component: &str, key: &str) -> Option<&'static str> {
        match (component, key) {
            ...
            // Infor connection internals: resolved from a saved Infor
            // connection (duckle-secrets) into props before planning, never a
            // typed form field.
            ("src.infor" | "snk.infor", "inforApiBase") => {
                Some("provided by the resolved Infor connection, not a form field")
            }
            ("src.infor" | "snk.infor", "inforTenant") => {
                Some("provided by the resolved Infor connection, not a form field")
            }
            // Structured upload config written by the Infor uploader editor
            // (InforUploadWorkspace.tsx), the same pattern as the xf.map mapper.
            ("snk.infor", "mapping" | "trimAlpha") => {
                Some("written by the Infor uploader editor")
            }
            _ => None,
        }
    }
        ...
        for (n, &start) in starts.iter().enumerate() {
            ...
            for m in read.find_iter(&body) {
                ...
                if names.iter().any(|k| union.contains(k) || UNIVERSAL.contains(&k.as_str()))
                    || names.iter().any(|k| ids.iter().any(|id| arm_allowed(id, k).is_some()))
                {
                    continue;
                }
